...
This script is designed to sit *after* the sentiment feature-building stage. It reads
issue-level and comment-level sentiment features from data/features/sentiment,
optionally enriches them with issue-set metadata, produces a set of analysis-ready
plots/tables, and writes a Markdown summary report.
...
python build_sentiment_analysis_report.py \
  --issue-features data/features/sentiment/issue_sentiment_features.parquet \
...
DEFAULT_ISSUE_FEATURES = "data/features/sentiment/issue_sentiment_features.parquet"
DEFAULT_COMMENT_FEATURES = "data/features/sentiment/comment_sentiment_features.parquet"
DEFAULT_WONTFIX_SET = "data/final/issue_sets/wontfix_issue_set.parquet"
DEFAULT_COMPARISON_SET = "data/final/issue_sets/comparison_issue_set.parquet"
DEFAULT_ISSUES_RESOLVED = "data/linked/resolved_entities/issues_resolved.parquet"
DEFAULT_REPOSITORIES = "data/processed/repositories.parquet"
...
class Paths:
    issue_features: Path
    comment_features: Path
    output_dir: Path
    wontfix_set: Path | None = None
    comparison_set: Path | None = None
    issues_resolved: Path | None = None
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--issue-features", default=DEFAULT_ISSUE_FEATURES)
    parser.add_argument("--comment-features", default=DEFAULT_COMMENT_FEATURES)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--wontfix-set", default=DEFAULT_WONTFIX_SET)
    parser.add_argument("--comparison-set", default=DEFAULT_COMPARISON_SET)
    parser.add_argument("--issues-resolved", default=DEFAULT_ISSUES_RESOLVED)
    parser.add_argument("--repositories", default=DEFAULT_REPOSITORIES)
...
def load_and_prepare(paths: Paths, winsorize: bool = False, exclude_zero_text_issues: bool = False) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, object]]:
    issue_df = pd.read_parquet(paths.issue_features)
    comment_df = pd.read_parquet(paths.comment_features)

    qa: dict[str, object] = {
        "issue_feature_rows_raw": int(len(issue_df)),
        "comment_feature_rows_raw": int(len(comment_df)),
    }

    issue_df = normalize_issue_features(issue_df)
    comment_df = normalize_comment_features(comment_df)

    qa["issue_feature_rows_normalized"] = int(len(issue_df))
    qa["comment_feature_rows_normalized"] = int(len(comment_df))
    qa["issue_duplicates"] = int(issue_df.duplicated(subset=["repo_full_name", "issue_id", "issue_number"]).sum())
    qa["comment_duplicates"] = int(comment_df.duplicated(subset=["repo_full_name", "issue_id", "issue_number", "comment_id"]).sum())
    ...
    # Optional enrichment from issue sets / resolved issues / repos
    issue_df = enrich_issue_features(issue_df, paths)

    # attach repository-level standardization for core variables
    for col in [c for c in PRIMARY_CONTINUOUS_FEATURES if c in issue_df.columns]:
        issue_df[f"repo_z_{col}"] = standardize_within_group(issue_df, col, group_col="repo_full_name")

    # derived indicators
    if "min_comment_sentiment" in issue_df.columns:
        issue_df["has_strongly_negative_comment"] = issue_df["min_comment_sentiment"] < NEGATIVE_THRESHOLD
    if "max_comment_sentiment" in issue_df.columns:
        issue_df["has_strongly_positive_comment"] = issue_df["max_comment_sentiment"] > POSITIVE_THRESHOLD
    if "comment_sentiment_change_late_minus_early" in issue_df.columns:
        issue_df["late_more_negative_than_early"] = issue_df["comment_sentiment_change_late_minus_early"] < 0
    if "std_comment_sentiment" in issue_df.columns and issue_df["std_comment_sentiment"].notna().any():
        cutoff = issue_df["std_comment_sentiment"].quantile(0.75)
        issue_df["high_sentiment_volatility"] = issue_df["std_comment_sentiment"] >= cutoff
    if {"max_comment_sentiment", "min_comment_sentiment"}.issubset(issue_df.columns):
        issue_df["comment_sentiment_range"] = issue_df["max_comment_sentiment"] - issue_df["min_comment_sentiment"]

    # optional winsorization for selected heavy-tailed metrics
    if winsorize:
        for col in [c for c in [*PRIMARY_CONTINUOUS_FEATURES, *PARTICIPATION_FEATURES] if c in issue_df.columns]:
            issue_df[f"winsor_{col}"] = winsorize_series(issue_df[col])

    if exclude_zero_text_issues and "comments_with_text_count" in issue_df.columns:
        before = len(issue_df)
        issue_df = issue_df[issue_df["comments_with_text_count"].fillna(0) > 0].copy()
        qa["issues_removed_zero_text_filter"] = int(before - len(issue_df))
...
def normalize_issue_features(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    ...
    analysis_set_col = find_col(out, ["analysis_set"])
    ...
    for col in out.columns:
        if any(token in col for token in ["count", "share", "ratio", "sentiment", "length", "slope"]):
...
def main() -> None:
    ...
    set_plot_style()
    paths = Paths(
        issue_features=Path(args.issue_features),
        comment_features=Path(args.comment_features),
        output_dir=Path(args.output_dir),
        wontfix_set=Path(args.wontfix_set) if args.wontfix_set else None,
        comparison_set=Path(args.comparison_set) if args.comparison_set else None,
        issues_resolved=Path(args.issues_resolved) if args.issues_resolved else None,
        repositories=Path(args.repositories) if args.repositories else None,
    )
    ...
    if not paths.issue_features.exists():
        raise FileNotFoundError(f"Issue feature file not found: {paths.issue_features}")
    if not paths.comment_features.exists():
        raise FileNotFoundError(f"Comment feature file not found: {paths.comment_features}")
    ...
    manifest = {
        "issue_features": str(paths.issue_features),
        "comment_features": str(paths.comment_features),
